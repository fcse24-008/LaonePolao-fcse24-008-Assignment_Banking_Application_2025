...
public class InvestmentAccount extends Account
{
    ...
    {
        return "InvestmentAccount= " + "accountID: " + getAccountID() + '\'' + ", balance: " + getBalance() + ", branch: " + getBranch() + '\''  + "jointCustomer=" + jointCustomer.getCustomerID() + ", businessCustomer=" + businessCustomer.getCustomerID() + ", individualCustomer=" + individualCustomer.getCustomerID() + '}';
    }
        ...
        if (conn != null) {
            try {
                PreparedStatement stmt = conn.prepareStatement(
                        "INSERT INTO investment_account (accountID, balance, branch, JCustomerID, BCustomerID, ICustomerID) VALUES (?, ?, ? ,? ,? ,?)"
                );
    ...
    public boolean deposit(double amount) {
        ...
        try (Connection conn = DBConnector.connect();
             PreparedStatement stmt = conn.prepareStatement(
                     "UPDATE investment_account SET balance = ? WHERE accountID = ?")) {
    ...
    public void creditInvestmentInterest() {
        ...
        try (Connection conn = DBConnector.connect();
            ...
            stmt.setInt(2, this.getAccountID());
            stmt.executeUpdate();
            System.out.println("✅ Investment interest credited: " + interest);
        } catch (SQLException e) {
            System.out.println("❌ Error crediting investment interest: " + e.getMessage());
        }
    }
}
